function obtenerComprobanteFirmado_sri(ruta_certificado, pwd_p12, ruta_respuesta, ruta_factura, host_email, email, passEmail, port, host_bd, pass_bd, user_bd, database, port_bd, id_factura) {

    path_host = window.location.origin;
    var response = [];
    $.ajax({
        url: path_host+"/js/lib_firma_sri/src/leerFactura.php",
        type: 'POST',
        data: {
        ...
        oReq.onload = function (oEvent) {
            ...
            $.ajax({
                url: path_host + "/js/lib_firma_sri/src/firma.php",
                type: 'POST',
                data: {
                ...
                $.ajax({
                    type: 'POST',
                    url: path_host + "/js/lib_firma_sri/src/services/validarComprobante.php",
                    data: {
                        'service': service, 'claveAcceso': $claveAcceso.text(),
                        'host_bd': host_bd, 'pass_bd': pass_bd,
                        'user_bd': user_bd, 'database': database,
                        'port_bd': port_bd, 'id_factura': id_factura,
                    },
                    context: document.body
                    ...
                    if (/RECIBIDA/i.test(respuesta) || /CLAVE ACCESO REGISTRADA/i.test(respuesta)) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: path_host + "/js/lib_firma_sri/src/services/autorizacionComprobante.php",
                            data: {
                                'service': service, 'claveAcceso': $claveAcceso.text(), 'host_email': host_email, 'email': email, 'passEmail': passEmail, 'port': port,
                                'host_bd': host_bd, 'pass_bd': pass_bd,
                                'user_bd': user_bd, 'database': database,
                                'port_bd': port_bd, 'id_factura': id_factura,
                            },
                            ...
                            alert(respuestaAutorizacionComprobante)
...
function fechas_certificado(ruta_certificado, mi_pwd_p12, ruta_respuesta) {
    ...
    oReq.onload = function (oEvent) {
        ...
        $.ajax({
            type: 'POST',
            url: "http://127.0.0.1:8000/js/lib_firma_sri/src/validarFechaCertificado.php",
            data: {
