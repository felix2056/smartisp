...
var chart;
		...
		$.ajax({
           ...
           chart = new Highcharts.Chart({
			   chart: {
				...
				events: {
					load: function () {
					traflan = setInterval(function () {
							requestDatta($('#val').val());
						}, 1000);
					}				
			}
		 },
  ...
  $('#info-router').on('hidden.bs.modal', function () {
  		clearInterval(traflan);
  		$('#trafico').hide();
  });

  $('#lgs,ifrouter').click(function(event) {
  		clearInterval(traflan);
  		$('#trafico').hide();
  });
