...
class generarPDF {
    ...
    public function facturaPDF($document, $claveAcceso,$host_email,$email_origen,$passEmail,$port,$fechaAutorizacion) {
        ...
        $pdf->RoundedRect(10, 80, 85, 50, 5, '1234');
    //    $host_img = $_SERVER["HTTP_HOST"]."/assets/img/logo_f.png";
//$pdf->Cell(40, 10, 'Hello World!');
        ...
        $pdf->image('uploads/Logo.jpg', null, null, 80, 40);
        $pdf->SetFillColor(255, 255, 255);
        $pdf->RoundedRect(110, 25, 85, 105, 5, '1234');
        $pdf->SetXY(112, 30);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetFont('Arial', '', 13);
        $pdf->MultiCell(100, 10, "RUC: " . $document->infoTributaria->ruc, 0);
        $pdf->SetXY(112, 40);
        $pdf->SetFont('Arial', '', 13);
        $pdf->MultiCell(100, 10, "FACTURA ", 0);
        $pdf->SetXY(112, 50);
        $pdf->SetFont('Arial', '', 8);
        $pdf->MultiCell(100, 10, "Nro:" . $document->infoTributaria->estab . '-' . $document->infoTributaria->ptoEmi . '-' . $document->infoTributaria->secuencial, 0);

        $pdf->SetXY(112, 60);
        $pdf->SetFont('Arial', '', 13);
        $pdf->MultiCell(70, 10, 'NUMERO DE AUTORIZACION: ', 0, 0, "C", true);
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY(112, 70);
//        $pdf->MultiCell(70, 10, $claveAcceso, 0, 0, "C", true);
        $pdf->Cell(80, 10, $claveAcceso, 0, 0, "L", true);
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY(112, 75);
        ...
        $pdf->SetXY(112, 85);
        $pdf->MultiCell(112, 10, 'Fecha Autorizacion:'.$fechaAutorizacion, 0);
        $pdf->SetXY(13, 85);
       // $pdf->RoundedRect(10, 15, 95, 76, 3, '', '1234');
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->MultiCell(100, 10, $document->infoTributaria->razonSocial, 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetXY(13, 95);
        $pdf->MultiCell(100, 10,"Direccion:", 0);
        $pdf->SetXY(13, 100);
        $pdf->MultiCell(100, 10, substr($document->infoTributaria->dirMatriz,'0',50));
        $pdf->SetXY(13, 105);
        $pdf->MultiCell(100, 10, substr($document->infoTributaria->dirMatriz,'50',50));
        $pdf->SetXY(13, 110);
        $pdf->MultiCell(100, 10, $contabilidad, 0);
        
        if($document->infoTributaria->contribuyenteRimpe){
            $pdf->SetXY(13, 115);
            $pdf->MultiCell(100, 10, utf8_decode('CONTRIBUYENTE RÉGIMEN RIMPE'), 0);
        }
        
        if($document->infoTributaria->agenteRetencion){
            $pdf->SetXY(13, 120);
            $pdf->MultiCell(100, 10, 'Agente Retencion: '.$document->infoTributaria->agenteRetencion, 0);
        }
        //Codigo de barras
        $pdf->SetXY(113, 95);
        $pdf->SetFont('Arial', '', 12);
        $pdf->MultiCell(60, 10, 'CLAVE DE ACCESO: ', 0, 0, "C", true);
        $pdf->SetXY(112, 102);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(80, 10, $claveAcceso, 0, 0, "L", true);
        $pdf->SetXY(113, 110);
        $this->generarCodigoBarras($claveAcceso);
        $pdf->image('uploads/codigo_mod.png', null, null, 78, 15);
        $pdf->SetXY(112, 115);
        $pdf->SetFillColor(255, 255, 255);
        $pdf->SetTextColor(0, 0, 0);
       ...
       // $pdf->RoundedRect(10, 80, 85, 40, 5, '1234');
       ...
       /* foreach ($document->infoFactura->pagos->pago as $e => $f) {
            if ($f->formaPago == '01') {
                $pdf->SetXY(13, $ejeX);
                $pdf->Cell(20, 10, 'Sin utilizacion del sistema financiero', 0, 0, "L");
                $ejeX = $ejeX + 6;
                $pdf->SetXY(13, $ejeX);
                $pdf->Cell(30, 10, 'Total: ' . $f->total, 0, 0, "L", true);
                $ejeX = $ejeX + 6;
                $pdf->SetXY(13, $ejeX);
                $pdf->Cell(30, 10, 'Plazo: ' . $f->plazo, 0, 0, "L", true);
                $ejeX = $ejeX + 6;
                $pdf->SetXY(13, $ejeX);
                $pdf->Cell(30, 10, 'Unidad de tiempo: ' . $f->unidadTiempo, 0, 0, "L", true);
            }
            if ($f->formaPago == '15') {
                $pdf->SetXY(22, $ejeX);
        ...
        */
        ...
        $ejeX = $ejeX + 45;
        $pdf->SetFillColor(255, 255, 255);
        $pdf->RoundedRect(10, $ejeX, 185, 21, 0, '1234');
        $ejeX = $ejeX + 2;
        $pdf->SetXY(20, $ejeX);
        $pdf->Cell(30, 10, 'Razon Social / Nombres y Apellidos:', 0, 0, "C", true);
        $pdf->SetXY(90, $ejeX);
        $pdf->Cell(30, 10, $document->infoFactura->razonSocialComprador, 0, 0, "C", true);
        $pdf->SetXY(160, $ejeX);
        $pdf->Cell(30, 10, 'RUC / CI:'.$document->infoFactura->identificacionComprador, 0, 0, "C", true);
        $ejeX = $ejeX + 8;
        $pdf->SetXY(12, $ejeX);
        $pdf->Cell(30, 10, 'Fecha Emision:'.$document->infoFactura->fechaEmision, 0, 0, "L", true);

        //detalle de la factura
        $pdf->SetXY(10, $ejeX + 20);
        $pdf->SetFillColor(36, 112, 13);
        ...
        foreach ($document->detalles->detalle as $a => $b) {
            ...
            $iva_precio = number_format(floatval($b->precioUnitario),2) * 0.12;
            $iva_precio = number_format($iva_precio,2);
            $iva_precio = $iva_precio * number_format(floatval($b->cantidad), 2);

            $total_iva += number_format($iva_precio,2);
        ...
        //$iva = floatval($subtotal12[0]) * 0.12;
        $total = $total_iva +floatval($subtotal12[0]);

        $pdf->SetXY(120, $ejeX + 20);
        $pdf->Cell(50, 5, "Subtotal 12%: ", 1, 0, "L", true);
        $pdf->SetXY(170, $ejeX + 20);
        $pdf->Cell(25, 5, $subtotal12, 1, 0, "R", true);
        $pdf->SetXY(120, $ejeX + 25);
        $pdf->Cell(50,5, "SubTotal 0%: ", 1, 0, "L", true);
        $pdf->SetXY(170, $ejeX + 25);
        $pdf->Cell(25,5, $subtotal0, 1, 0, "R", true);
        $pdf->SetXY(120, $ejeX + 30);
        $pdf->Cell(50, 5, "SubTotal no sujeto de IVA: ", 1, 0, "L", true);
        $pdf->SetXY(170, $ejeX + 30);
        $pdf->Cell(25, 5,number_format( $subtotal_no_impuesto, 2, ',', ' '), 1, 0, "R", true);
        $pdf->SetXY(120, $ejeX + 35);
        $pdf->Cell(50, 5, "SubTotal exento de IVA: ", 1, 0, "L", true);
        $pdf->SetXY(170, $ejeX + 35);
        $pdf->Cell(25, 5, number_format( $subtotal_no_iva, 2, ',', ' '), 1, 0, "R", true);
        $pdf->SetXY(120, $ejeX + 40);
        $pdf->Cell(50, 5, "SubTotal sin Impuestos: ", 1, 0, "L", true);
        $pdf->SetXY(170, $ejeX + 40);
        $pdf->Cell(25, 5, number_format(floatval($document->infoFactura->totalDescuento), 2, '.', ' '), 1, 0, "R", true);
        $pdf->SetXY(120, $ejeX + 45);
        $pdf->Cell(50, 5, "Descuento: ", 1, 0, "L", true);
        $pdf->SetXY(170, $ejeX + 45);
        $pdf->Cell(25, 5, number_format(floatval($document->infoFactura->totalDescuento), 2, ',', ' '), 1, 0, "R", true);
        $pdf->SetXY(120, $ejeX + 50);
        $pdf->Cell(50, 5, "IVA 12%: ", 1, 0, "L",true);
        $pdf->SetXY(170, $ejeX + 50);
        $pdf->Cell(25, 5, $iva, 1, 0, "R",true);
        $pdf->SetXY(120, $ejeX + 55);
        $pdf->Cell(50, 5, "ICE: ", 1, 0, "L",true);
        $pdf->SetXY(170, $ejeX + 55);
        $pdf->Cell(25, 5,number_format(floatval( $ice), 2, ',', ' '), 1, 0, "R",true);
        $pdf->SetXY(120, $ejeX + 60);
        $pdf->Cell(50, 5, "IRBPNR: ", 1, 0, "L",true);
        $pdf->SetXY(170, $ejeX + 60);
        $pdf->Cell(25, 5, number_format(floatval($IRBPNR), 2, ',', ' '), 1, 0, "R",true);
        $pdf->SetXY(120, $ejeX + 65);
        $pdf->Cell(50, 5, "Valor Total: ", 1, 0, "L",true);
        $pdf->SetXY(170, $ejeX + 65);
        $pdf->Cell(25, 5, $document->infoFactura->importeTotal, 1, 0, "R",true);
        ...
        $pdf->SetFillColor(192);
        $ejeY = $ejeX+30;
        $pdf->RoundedRect(10, $ejeY, 100, 20, 0, '1234');
        $pdf->SetXY(10, $ejeX + 20);
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->MultiCell(100, 30, "INFORMACION ADICIONAL:", 0);
        $pdf->SetXY(10, $ejeX + 40);
        $pdf->SetFont('Arial', '', 7);
        $pdf->MultiCell(100, 5, "" . $infoAdicional . "", 0);


        $pdf->RoundedRect(10, $ejeX+50, 100, 20, 0, '1234');
        $pdf->SetXY(10, $ejeX+40);
        $pdf->SetFont('Arial', 'B', 7);
        $pdf->MultiCell(100, 30, "FORMAS DE PAGO:", 0);
        $pdf->SetFont('Arial', '', 7);
        $ejeX = $ejeX + 50;
        $ejeX = $ejeX + 5;
        $pdf->SetXY(10, $ejeX);
        foreach ($document->infoFactura->pagos->pago as $e => $f) {
            if ($f->formaPago == '01') {
                $pdf->SetXY(10, $ejeX);
                $pdf->MultiCell(100, 10, 'Otros sin utilizacion del sistema financiero',0);
                $ejeX = $ejeX + 6;
                $pdf->SetXY(10, $ejeX);
                $pdf->MultiCell(100, 10, 'Total: ' . $f->total,0);
//                $ejeX = $ejeX + 6;
//                $pdf->SetXY(13, $ejeX);
//                $pdf->MultiCell(100, 10, 'Plazo: ' . $f->plazo,0);
//                $ejeX = $ejeX + 6;
//                $pdf->SetXY(13, $ejeX);
//                $pdf->MultiCell(100, 10, 'Unidad de tiempo: ',0);
            }
            if ($f->formaPago == '15') {
                $pdf->SetXY(22, $ejeX);
                $pdf->Cell(30, 10, 'Compensacion de deudas', 0, 0, "C", true);
	...
	$pdf->Cell(25, 10,"Para la atencion de reclamos NO resueltos por el prestador, ingrese su reclamo al link o para informacion a http://reclamoconsumidor.arcotel.gob.ec/osTicket/" , 0, 0, "C");
        
        $ejeX = $ejeX + 10;
        
        $pdf->SetXY(85, $ejeX);

	$pdf->Cell(25, 10,"o comuniquese con el numero telefonico 1800 567 567" , 0, 0, "C");
	
        $pdf->Output('../../comprobantes/' . $claveAcceso . '.pdf', 'F');
        $email = new sendEmail();
        $email->enviarCorreo('Factura', $document->infoFactura->razonSocialComprador, $claveAcceso, $correo,$host_email,$email_origen,$passEmail,$port);
    }
...
//$pdf->Cell(40, 10, 'Hello World!');
        ...
        $pdf->SetXY(10, 0);
        $pdf->image($host_img, null, null, 80, 30);
