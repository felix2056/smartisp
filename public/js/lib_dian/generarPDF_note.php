...
$pdf->Cell(0,10,'NOTA '.utf8_decode(($_POST['note_type'])?'CRÉDITO':'DÉBITO'),'LRB',0,'C',false);$pdf->Ln();
// Color and font restoration
$pdf->SetFillColor(240,240,240);
$pdf->SetTextColor(0);
$pdf->SetFont('');
$pdf->SetFont('Times','',9);
$pdf->Cell(0,10,utf8_decode('CUDE: '.$_POST['cude']),'LRB',0,'C',false);$pdf->Ln();
$pdf->lineColor('Datos del Documento','Número de Nota '.($_POST['note_type'])?'Crédito':'Débito'.': '.$_POST['prefix'].$_POST['number'],14,7);        
$datos=[
...
if ($_POST['detalle']!='') {
    foreach ($_POST['detalle'] as $d) {
        $pdf->Cell(20, 8, $d['plan_id'], 1, 0, "C", false);
        $pdf->Cell(55, 8, $d['description'], 1, 0, "C", false);
        $pdf->Cell(20, 8, $d['quantity'], 1, 0, "C", false);
        $pdf->Cell(25, 8, $d['price'], 1, 0, "C", false);
        $pdf->Cell(25, 8, $d['subtotal'], 1, 0, "C", false);
        $pdf->Cell(20, 8, $d['total_iva'], 1, 0, "C", false);
        $pdf->Cell(25, 8, $d['total'], 1, 0, "C", false);
        $pdf->Ln();
    }
}
$pdf->Cell(190,6,utf8_decode('Datos de Referencia'),1,1,'L',true);
$datos=[
    ['Tipo de Documento Referencia: Factura Electrónica','LT',false],
    ['','TR',false],
    ['Fecha Referencia: '.$_POST['dateFactura'],'L',false],
    ['Número de Referencia: '.$_POST['numberFactura'],'R',false]
];
$pdf->lineNormal($datos);
...
$filename = $PNG_TEMP_DIR.'test'.md5($_POST['cude']).'.png';
...
if ($_POST['typeoperation_cod']=='2') {
    $pdf->Cell(0,20,utf8_decode('ESTA DOCUMENTO ES DE PRUEBA Y NO TIENE VALIDEZ FISCAL'),7,0,'C',false);
}
$pdf->Output('comprobantes_colombia/'.$_POST['cude'].'.pdf', 'F');
//Enviamos el correo con el XML y el PDF
//$email = new sendEmail();
//$email->enviarCorreo('Nota', 'Comprador SAS', $_POST['filename'],$_POST['correo'],$_POST['host_email'],$_POST['email_origen'],$_POST['passEmail'],$_POST['port']);