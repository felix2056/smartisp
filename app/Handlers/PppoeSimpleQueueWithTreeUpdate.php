...
class PppoeSimpleQueueWithTreeUpdate implements ShouldQueue
{
     ...
     * @return void
     */
    public function handle(\App\Events\PppoeSimpleQueueWithTreeUpdate $event)
    {
        set_time_limit(0);
        if ($event->API->connect($event->con['ip'], $event->con['login'], $event->con['password'])){
            ...
            $UPDATE = $rocket->update_ppp_simple_queue_with_tree($event->API,$event->data,$event->oldtarget,$event->data['newtarget'],$gat->gateway,$event->debug);
