...
class AddClientsOnNewRouters implements ShouldQueue
{
     ...
     * @return void
     */
    public function handle(\App\Events\AddClientsOnNewRouter $event)
    {
        ...
        try {
            ...
            if ($API->connect($con['ip'], $con['login'], $con['password'])) {
                if (count($clients) > 0) {
                    ...
                    if ($control == 'pc' || $control == 'pa' || $control == 'rp') {
    ...
    public function control_migrate_process($API,$rocket,$router_id,$options,$oldcontrol,$control,$debug){
         ...
         * crear un registro del radius en la base de datos con los datos de conexion
         * crear una nueva conexion dinaica a db
         * crear el nas en la bd de radius
         * reiniciar el servicio de freradius para que tome el nas
         * habilitar pppoe use radius por api de mkt
         * crear radius en mkt (con el secret)
         **/
        ...
        foreach ($clients as $client) { //iteramos los clientes
            ...
            switch ($control) {
                ...
                case 'pa':
                    ...
                    break;
                /**nueva forma de control agregada 18/08/2021 para pppoe con radius**/
                case 'ra':

                    /**por cada radreply cliente insertamos ancho de banda  y la ip que debería devolver el radius para la conexion**/
          /*          $radreply_ip = Radreply::create([
                        'username' => $client->user_hot,
                    ...
                    $radreply_bandwidth = Radreply::create([
                        ...
                        'op' => '=',
                        'value' => $plan['upload'].'k/'.$plan['download'].'k'
                    ]);*/
                    ...
                    $radcheck_pass = Radcheck::create([
                        ...
                        'op' => ':=',
                        'value' => $client->pass_hot == '0' ? '0': $en->decode($client->pass_hot)
                    ]);

                    /**$client->pass_hot == '0' ? '0': $en->decode($client->pass_hot) password**/
                ...
                case 'rr':
                    ...
                    /**si le tiene que aplicar el control por mkt segun el plan**/
                    if(!$plan['no_rules'])
                        $ADD = $rocket->add_ppp_simple($API,$data,$client->ip,$gat->gateway,$debug,true);

                   /**por reunion con Jerson, desactivamos el secret de mkt cuando es radius**/
                    /* if ($drop==1) {
                        //bloqueamos definitivo si esta activo el drop
                        $PPP = Ppp::ppp_get_id($API,$data['user']);
                    ...
                    /**cuando lo agregamos, realizamos una desconexion del servicio. Estol es porque en las otras topologias no desactivan la conexion, por lo cual queda abierta
                     * entonces cuando el cliente se quiere conectar por pppoe ya tiene una conexion entonces no realiza la peticion a radius.
                     si no realiza la peticion a radius, no se le crea la cola simple*
                     */
    ...
    public function addSimpleQueueWithTree($API,$error,$data,$client,$rocket,$debug)
    {
        ...
        if ($parent=='notFound') {
            # Creamos parent
            $a = SimpleQueuesTree::add_simple_parent($API,$dataNamePlan,$P_DATA['ips'],$P_DATA['maxlimit'],$P_DATA['bl'],$P_DATA['bth'],$data['bt'],$P_DATA['limitat'],$data['priority'],$dataNamePlan);
