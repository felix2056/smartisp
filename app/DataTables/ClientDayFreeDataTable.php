...
class ClientDayFreeDataTable extends DataTable
{
    protected $global;
    protected $campos_v;
                ...
                if ($row->service->count() > 0) {
                    foreach ($row->service as $service) {
                        if ($service->status == 'ac') {
                            $actions .= $styleb . '<a class="blue ban-service" href="#" id="' . $service->id . '" title="' . __('app.serviceCut') . '" xmlns="http://www.w3.org/1999/html"><i class="ace-icon fa fa-adjust bigger-130"></i></a>' . $editar . $eliminar . '<a class="grey tool" title="' . __('app.tools') . '" href="#" id="' . $service->id . '"><i class="ace-icon fa fa-wrench bigger-130"></i></a><a class="blue infos" title="' . __('app.information') . '" href="#" id="' . $service->id . '"><i class="ace-icon fa fa-info-circle bigger-130"></i></a></div></br>';
                        }
                        if ($service->status == 'de') {
                            $actions .= $styleb . '<a class="blue ban-service" href="#" id="' . $service->id . '" title="' . __('app.activate') . ' ' . __('app.service') . '"><i class="ace-icon fa fa-adjust bigger-130"></i></a>' . $eliminar . '<a class="blue infos" title="' . __('app.information') . '" href="#" id="' . $service->id . '"><i class="ace-icon fa fa-info-circle bigger-130"></i></a></div></br>';
                        }
                    }
                } else {
...
//                return $expiration;
    ...
    {
        return Client::with('service', 'service.router', 'service.router.control_router', 'service.plan')
            ...
            ->select('clients.id', 'clients.name', 'clients.email', 'clients.dni', 'clients.balance', 'billing_settings.billing_grace_period', 'billing_settings.billing_due_date', 'billing_settings.billing_date', 'zone.name as zone', 'odb_splitter.name as odb_id', 'onu_type.onutype as onu_id', 'clients.onusn')
        ...
        return $this->builder()
            ...
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["day-free-table"].buttons().container()
    ...
    {
        return [
            "Name" => ['data' => 'name', 'name' => 'name', 'orderable' => true, 'searchable' => true, 'visible' => (bool)$this->campos_v->name],
    ...
    {
        return 'Client_day_free_' . date('YmdHis');
    }
