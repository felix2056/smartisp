...
class ActivateDeactivatedClients extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'activate-deactivated-clients';
...
//		                $dia_corte = $client->billing_settings->billing_due_date;
...
//		                $fecha = $client->billing_settings->getBillingDueDatesAttribute_2();
//		                $nuevafecha = strtotime('+' . $dia_corte . ' day', strtotime($fecha));
//		                $nuevafecha = date('Y-m-d', $nuevafecha);
//		dd($billingDueDate);
//		                if ((float)$client->balance <= 0) {
//			                if (now()->startOfDay()->lessThan($billingDueDate->startOfDay())) {
						        if (now()->startOfDay()->lessThan($billingDueDate->startOfDay()) || $cortadoDetails['paid'] || (!$cortadoDetails['paid'] && $cortadoDetails['amount'] < 1)) {
