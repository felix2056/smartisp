...
class SecuencialesController extends Controller {
    ...
    public function getIndex() {
        ...
        if ($level == 'ad' || $access == true) {
            ...
            $secuenciales = Secuenciales::select('id', 'comprobante', 'valor', 'updated_at')->get();



            $data['factura'] = [
                'quantity' => $secuenciales->count(),
                'total' => 0
            ];
            ...
            if (Auth::user()->level == 'ad')
	            ...
	            $contents = View::make('secuenciales.index', $permissions);
    ...
    public function postLists() {
        $invoices = Secuenciales::select('id', 'comprobante', 'valor', 'updated_at')->get();

        return DataTables::of($invoices)
                        ->addColumn('action', function ($row) {
                            return '
                    <a href="javasscript:;" title="Remove" onclick="updateSecuencia(\'' . $row->id . '\');return false;"><span class="glyphicon glyphicon-pencil"></span></a>';
                        })
    ...
    public function showInvoice($id) {
        $invoice = Secuenciales::select('id', 'comprobante', 'valor', 'updated_at')
                ->where('id', $id)
                ->first();



        return view('secuenciales.show', ['invoice_data' => $invoice]);
    }

    public function save($request) {


        Secuenciales::where('id', 1)
          ->update(['valor' =>$request]); // this will also update the record
        return Reply::success('Sequence successfully updated.');
    }
