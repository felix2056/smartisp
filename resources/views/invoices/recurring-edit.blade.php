<div class="modal-header">
    ...
    <h4 class="modal-title" id="myModalLabel">@lang('app.editRecurringInvoice')</h4>
</div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-9 col-md-9">
                            <input
                                ...
                                type="text"
                                autocomplete="false"
                                name="start_date" value="{{ $invoice->start_date->format('d/m/Y') ?? '' }}">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="invoice_date" class="control-label col-lg-3 col-md-3">
                        ...
                        <div class="col-lg-9 col-md-9">
                            <input
                                ...
                                autocomplete="false"
                                name="end_date"
                                value="{{ $invoice->end_date ? $invoice->end_date->format('d/m/Y') : ''}}"
                            >
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-lg-9 col-md-9">
                            <select class="form-control" name="frequency" id="frequency">
                                <option value="week" @if($invoice->frequency == 'week') selected @endif>Weekly</option>
                                <option value="month" @if($invoice->frequency == 'month') selected @endif>Monthly</option>
                                <option value="year" @if($invoice->frequency == 'year') selected @endif>Yearly</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                            ...
                            <tbody class="ui-sortable">
                                @forelse($invoice->items as $key => $item)
                                    <tr class="rows ui-sortable-handle">
                                        ...
                                        <td>
                                            <input type="text" name="unit[]" value="{{$item->unit ?? ''}}" class="unit form-control input-sm" style="width:50px">
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <input type="text" name="with_tax[]" disabled class="with_tax decimal form-control input-sm" value="{{$item->total ?? ''}}" style="width:80px">
                                        </td>
                                        <td>
                                            <input type="text" name="total[]" readonly class="total decimal form-control input-sm" value="{{$item->total ?? ''}}" style="width:80px">
                                        </td>
                                        <td>
...
<div class="modal-footer">
    ...
    <button id="add_invoice" type="button" class="btn btn-primary" onclick="editInvoiceItems('{{ $invoice->client_id }}', '{{ $invoice->id }}'); return false;">Update</button>
</div>
...
<script>
    ...
    function editInvoiceItems(id, invoice) {
        var url = '{{ route('invoice.updateRecurringInvoice', [':id', ':invoice_id']) }}';
        url = url.replace(':id', id);
        url = url.replace(':invoice_id', invoice);
